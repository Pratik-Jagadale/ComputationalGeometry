...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER,stlmodel.stl.vertices.size() * sizeof(float), &(stlmodel.stl.vertices[0]), GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, stlmodel.stl.normals.size() * sizeof(float), &(stlmodel.stl.normals[0]), GL_STATIC_DRAW);
	glVertexAttribPointer(PVG_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(PVG_ATTRIBUTE_NORMAL);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void display(void)
{
	...
	scaleMatrix = vmath::scale(1.0f, 1.00f, 1.00f);
	...
	*/
	...
	glBindVertexArray(vao_Sphere);
	glDrawArrays(GL_TRIANGLES, 0, stlmodel.stl.vertices.size());
	glBindVertexArray(0);


	/* glBindVertexArray(stlmodel.boundingBox.vao);
	glDrawArrays(GL_LINES, 0, stlmodel.boundingBox.size);
	glBindVertexArray(0); */
...
/* glBindVertexArray(stlmodel.boundingBox.vao);
	...
	glBindVertexArray(stlmodel.boundingBox.vao);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, stlmodel.boundingBox.ibo);
	glDrawElements(GL_TRIANGLES, stlmodel.boundingBox.size, GL_UNSIGNED_SHORT, 0);
	glBindVertexArray(0);
